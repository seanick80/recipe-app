...
// gluestack-ui v4-alpha's icon interop mapping does not satisfy nativewind
// 4.2.6's stricter `nativeStyleToProp` type; the runtime shape is correct,
// only the types drift, so we cast the options object.
cssInterop(UIIcon, {
  className: {
    target: 'style',
    nativeStyleToProp: {
...
>(({ className, size, ...props }, ref) => {
  ...
  // `size` here is ActivityIndicator's ('small' | 'large' | number); the tva
  // slot types it as the button size variant. Cast keeps runtime behavior.
