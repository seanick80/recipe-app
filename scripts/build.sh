...
info "verifying Models/ files referenced by iOS sources are in codemagic.yaml copy list"
# The iOS app compiles on Codemagic, which has no view of the root Models/
...
# Forgetting to add a new Models/*.swift here produces a "cannot find type
# X in scope" error that only surfaces on CI — so validate locally.
check_models_in_codemagic() {
    local missing="" f base syms referenced sym
    for f in Models/*.swift; do
        base="$(basename "$f" .swift)"
        case "$base" in
            Test*|Recipe|GroceryItem|ShoppingTemplate) continue ;;
        esac
        ...
        if [[ $referenced -eq 1 ]] && ! grep -Fq "Models/${base}.swift" codemagic.yaml; then
            missing="${missing}${base}.swift "
        fi
    done
    if [[ -n "$missing" ]]; then
        err "codemagic.yaml is missing these Models/ files from its 'Copy shared parser modules' step (iOS sources reference their symbols): ${missing}"
    fi
}
check_models_in_codemagic
